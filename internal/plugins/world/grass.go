...
var grassSprite = ebiten.NewImage(4, 12)
...
func (g *grass) Draw(camX, camY float64) {
	for _, blade := range g.blades {
		ebitenrenderutil.DrawAtRotated(blade.sprite, g.dst, blade.posX, blade.posY, blade.angle+blade.angleOffset, 0.5, 1.0)
	}
}
...
func NewGrass(
	...
	for i := 0; i < int(bladesPertile*entity.Width/gridSize); i++ {
		...
		newGrass.blades = append(newGrass.blades, newGrassBlade(x, entity.Px[1]-4))
	}
