...
func MakeJumpParticlesBundle(x, y float64, dir maths.Direction, halfSize float64) engine.Bundle {
	return func(cmd *commands.Commands, scene *engine.Scene) *engine.Node {
		...
		jumpParticles := scene.SpawnActor("JumpParticles", transform2D.NewTransform2D(
			transform2D.WithPos(x, y),
			transform2D.WithAngle(rot),
		), cmd)

		particlesBroad := particles.NewParticleSystem(
		...
		jumpParticles.AddChild(particlesBroad, "JumpParticlesBroad", engine.MakeOnTreeAdd(particlesBroad, cmd))

		particlesTight := particles.NewParticleSystem(
			//			particles.WithGraphic(
			//				graphic.NewGraphic(
			//					graphic.WithTransform(
			//						transform2D.NewTransform2D(
		...
		jumpParticles.AddChild(particlesTight, "JumpParticlesTight", engine.MakeOnTreeAdd(particlesTight, cmd))
